...
keep if (YEAR >= 2010 & MASTER_CARNEGIE_BASIC == 15 & (MASTER_CONTROL_FILLED == 0 | MASTER_CONTROL_FILLED == 1 | MASTER_CONTROL_FILLED == 2)) // "R1" universities only - research university very high research activity
...
collapse (mean) UG_COUNT_0_30 UG_COUNT_30_48 UG_COUNT_48_75 UG_COUNT_75_110 UG_COUNT_OVER_110 TOTAL_FTFT_UG (sum) W_UG_AVG_NET_PRICE_0_30 W_UG_AVG_NET_PRICE_30_48 W_UG_AVG_NET_PRICE_48_75 W_UG_AVG_NET_PRICE_75_110 W_UG_AVG_NET_PRICE_OVER_110, by(STATE MASTER_STATE YEAR)
...
gen UG_COUNT_0_30_SHARE = UG_COUNT_0_30 / TOTAL_FTFT_AID * 100
gen UG_COUNT_30_48_SHARE = UG_COUNT_30_48 / TOTAL_FTFT_AID * 100
gen UG_COUNT_48_75_SHARE = UG_COUNT_48_75 / TOTAL_FTFT_AID * 100
gen UG_COUNT_75_110_SHARE = UG_COUNT_75_110 / TOTAL_FTFT_AID * 100
gen UG_COUNT_OVER_110_SHARE = UG_COUNT_OVER_110 / TOTAL_FTFT_AID * 100
...
foreach state of local STATE_LIST {
	...
	scalar type_directory = "`=state_directory'/r1_only"
	...
	graph twoway line a2010 a2022 FAMILY_BUCKET, sort ytitle("Net Price ($)") legend(label(1 "2009-10 School Year") label(2 "2021-22 School Year") position(6) rows(1)) xtitle(" ") title("`=title' R1 Institutions Average Net Price for Full-Time," "First-Time Undergraduates Eligible for Title IV Aid" "by Family Income Bracket") xscale(range(0.5,5.5)) xscale(noline) xlabel(, valuelabel noticks nogrid) ylabel(0(`=r')`=max_y_scale', format(%15.0fc)) xmtick(0.5(1)5.5, grid noticks)

	graph export "`=type_directory'/net_price_by_income_bracket_two_years.png", replace

	graph bar b2010 b2022, over(FAMILY_BUCKET) legend(label(1 "2009-10 School Year") label(2 "2021-22 School Year") position(6) rows(1)) title("`=title' R1 Institutions Average # of Full-Time, First-Time" "Undergraduates Eligible for Title IV Aid Enrolled" "by Family Income Bracket") ylabel(, format(%15.0fc)) blabel(bar, format(%15.01fc)) note("`=share2010'% of full-time, first-time degree seeking undergraduates at `=title' received Title IV aid" "in 2009-10. `=share2022'% received aid in 2021-22.")

	graph export "`=type_directory'/enrollment_count_by_income_bracket_two_years.png", replace

	graph bar c2010 c2022, over(FAMILY_BUCKET) legend(label(1 "2009-10 School Year") label(2 "2021-22 School Year") position(6) rows(1)) title("`=title' R1 Institutions Average % of Full-Time, First-Time" "Undergraduates Eligible for Title IV Aid Enrolled" "by Family Income Bracket") ylabel(#5, format(%15.0fc)) blabel(bar, format(%15.2fc)) note("`=share2010'% of full-time, first-time degree seeking undergraduates at `=title' received Title IV aid" "in 2009-10. `=share2022'% received aid in 2021-22.")
