...
keep if (YEAR >= 2010 & (MASTER_CONTROL_FILLED == 0 | MASTER_CONTROL_FILLED == 1 | MASTER_CONTROL_FILLED == 2) & HBCU == 1) // HBCUs only
...
scalar directory = "${ipeds_path}/outputs/memo_7/hbcus"
...
graph twoway line a2010 a2022 FAMILY_BUCKET, sort ytitle("Net Price ($)") legend(label(1 "2009-10 School Year") label(2 "2021-22 School Year")) xtitle("") title("HBCUs" "Average Net Price by Family Income Bracket") xscale(range(0.5,5.5)) xscale(noline) xlabel(, valuelabel noticks nogrid) ylabel(#5, format(%15.0fc)) xmtick(0.5(1)5.5, grid noticks)
	
graph export "`=directory'/net_price_by_income_bracket_two_years.png", replace
	
graph bar b2010 b2022, over(FAMILY_BUCKET) legend(label(1 "2009-10 School Year") label(2 "2021-22 School Year") position(6) rows(1)) title("HBCUs" "Average # of First-Time Full-Year Students Enrolled by" "Family Income Bracket") ylabel(, format(%15.0fc)) blabel(bar, format(%15.01fc))
